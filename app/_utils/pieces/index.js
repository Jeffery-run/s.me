...
function extendSingle(target, source) {
  ...
  // for (const key in source) { target[key] = is.arr(source[key]) ? source[key].slice(0) : source[key]; }
  return target;
}

function extend(target, ...agrs) {
  if (!target) target = {};
  for (let i = 0; i < agrs.length; i++) { extendSingle(target, agrs[i]); }
  return target;
}

function pushArray(arr, arr2) {
  arr.push.call(arr, ...arr2);
}
...
Pieces.prototype = {
    ...
    if (options.items.length) {
      options.items.forEach((item) => {
        ...
        switch (item.type) {
          ...
          default: return alert('unknown item type');
        }
      });
    }
  ...
  initPieces(options) {
    ...
    let piecesWidth; let piecesHeight; let piecesSpacing; let item; let last; let p; let x; let lx; let y; let w; let
      h;
    this.pieces = [];
    this.items = [];
    const { itemSeparation } = options;
  ...
  clearRect(o) {
    const t = new Transform();
    let newRedraw = false; let x; let y; let w; let h; let p1; let p2; let p3; let p4; let left; let top; let right; let bottom; let itemRect; let angle; let rotate; let
      clear = null;
    ...
    this.items.forEach((item) => {
      ...
      if (this.isItemDifferent(item)) {
        ...
        item.pieces.forEach((p) => {
          ...
          itemRect = this.extendRect(itemRect, {
